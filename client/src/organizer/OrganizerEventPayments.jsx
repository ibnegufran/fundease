      ...
      <div className="bg-white rounded-xl shadow p-3">
        {loading ? (
          ...
          <ul className="divide-y">
            {filtered.map((p) => (
              <li key={p._id} className="py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                {/* left: student & meta */}
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      <p className="font-semibold text-gray-800 truncate">{p.studentName || "-"}</p>
                      <p className="text-xs text-gray-500 truncate">{p.rollNo || ""}</p>
                    </div>
                  ...
                  <div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs text-gray-600">
                    {/* UPI name (prominent) */}
                    <div>
                      <div className="text-[12px] text-gray-700 font-medium">UPI Name</div>
                      <div className="mt-0.5 truncate">{p.note || p.upiId || "-"}</div>
                    </div>

                    {/* Class */}
                    <div>
                      <div className="text-[12px] text-gray-700 font-medium">Class</div>
                      <div className="mt-0.5">{p.className || "-"}</div>
                    </div>

                    {/* Date & Time (prominent) */}
                    <div>
                      <div className="text-[12px] text-gray-700 font-medium">Paid On</div>
                      <div className="mt-0.5">{formatDateTime(p.createdAt)}</div>
                    </div>
                  </div>
                </div>

                {/* right: actions */}
                <div className="flex flex-col justify-center   items-center gap-4 mt-3 sm:mt-0">
                   <div className=" flex items-center  ">
                    ...
                    <div className="bottons flex gap-2">
                  <button
                    ...
                    } ${updating === p._id ? "opacity-60 cursor-not-allowed" : ""}`}
                  >
                    ✓ Verify
                  </button>

                  <button
                    onClick={() => updateStatus(p._1d ?? p._id, "Rejected")}
